...
export default async function handler(
    ...
    try {
        ...
        if (response.failureCount > 0) {
            // Processar falhas e remover tokens inválidos
            ...
            if (tokensToRemove.length > 0 && email) {
                console.log(`[Send Notify] Removendo ${tokensToRemove.length} tokens inválidos para ${email}`)
                const userDocRef = firestoreDb.collection('token-usuarios').doc(email)
